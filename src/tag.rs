...
pub fn parse_line(line: &str) -> Vec<(String, VoiceCtx)> {
    ...
    while let Some(c) = chars.next() {
        ...
        match classify_tag(inner.trim()) {
            TagKind::Unknown(raw) => {
                buf.push('['); buf.push_str(&raw); buf.push(']');
            }
            kind => {
                // タグ前のbufをフラッシュ
                let text = buf.trim().to_string();
                if !text.is_empty() { result.push((text, ctx.clone())); }
                buf.clear();
                match kind {
    ...
    let text = buf.trim().to_string();
    if !text.is_empty() { result.push((text, ctx)); }
    result
}
...
pub fn ctx_to_prefix(ctx: &VoiceCtx) -> String {
    ...
    if ctx.char_name != t.default_char {
        s.push('['); s.push_str(&ctx.char_name); s.push(']');
    }
    if ctx.style_name != t.default_style {
        s.push('['); s.push_str(&ctx.style_name); s.push(']');
    }
    s
}
    ...
    while let Some(c) = chars.next() {
        ...
        if let Ok(id) = tag.parse::<u32>() {
            if let Some((char_name, style_name)) = t.by_style_id.get(&id) {
                ...
                let char_default_style = t.char_styles.get(char_name)
                    .and_then(|v| v.first())
                    .map(|(s, _)| s.as_str())
                    .unwrap_or("");
